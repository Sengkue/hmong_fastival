...
const handleLogin = async () => {
  ...
  try {
    // DEV MODE: Bypass API if credentials are 'admin' / 'admin123'
    ...
    if (response.success) {
      if (response.token && response.user) {
         authStore.login(response.token, response.user, response.permissions || {})
         router.push('/')
      } else {
    ...
    errorMessage.value = error.response?._data?.message || 'Connection error. Please try again.'
